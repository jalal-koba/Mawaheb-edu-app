...
class _VideoProgressBarState extends State<CustomVideoProgressBar> {
  void listener() {
    if (!mounted) return;
    setState(() {});
  }
  ...
  void _seekToRelativePosition(Offset globalPosition) async{
   ...
   audioPlayer.seek(Duration(milliseconds:(context.calcRelativePosition(
      controller.value.duration,
      globalPosition,
    )).inMilliseconds));
  }
    ...
    return chewieController.draggableProgressBar
      ...
      onHorizontalDragUpdate: (DragUpdateDetails details) {
        ...
        if(_latestDraggableOffset != null) {
          duration = context.calcRelativePosition(
              controller.value.duration, _latestDraggableOffset!);
        }
...
class StaticProgressBar extends StatelessWidget {
  const StaticProgressBar({super.key, 
    required this.value,
